"""
Batch airborne event evaluation pipeline v3_1.

Uses v3_1 hybrid state machine: keeps v2's rolling deque as primary baseline,
but initializes with player-feet detection to avoid cold-start bias.

Output: outputs/airborne_eval_v5_v3_1/ (separate from v2 and v3)
"""
...
DEFAULT_OUT_DIR = ROOT / "outputs" / "airborne_eval_v5_v3_1"

sys.path.insert(0, str(ROOT))
sys.path.insert(0, str(ED_DIR))

from airborne_rule_v2 import AirborneRuleV2Config
from airborne_state_machine_v3_1 import (
    StateMachineV3_1Config,
    detect_airborne_events_with_state_machine_v3_1,
)
...
def run_clip(clip_name: str, out_dir: Path) -> dict | None:
    ...
    print(f"\n{'=' * 60}")
    print(f"  {clip_name} v3_1 (hybrid baseline: deque primary + player fallback)")
    print(f"{'=' * 60}")
    ...
    completed = detect_airborne_events_with_state_machine_v3_1(df, rule_config, sm_config)
...
def write_summary(all_results: list[dict], out_dir: Path):
    ...
    print(f"\n{'=' * 60}")
    print("CONSOLIDATED SUMMARY V3_1")
    print(f"{'=' * 60}")
