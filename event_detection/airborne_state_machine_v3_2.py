"""
Airborne state machine v3_2 (velocity-stationarity landing).

Extends v3_1 hybrid baseline with improved landing detection:
Instead of just "2 consecutive frames near baseline", require BOTH:
1. Ball near baseline (distance < 50px)
2. Velocity stationarity: |dy| < 2.0 px/frame for last 5 frames

This prevents false landings when ball briefly passes near baseline mid-arc
but is still moving with significant vertical velocity.

Fully causal (uses past frames only), no lookahead needed.
"""
...
class StateMachineV3_2Config:
    ...
    # V3_2: velocity-stationarity landing confirmation
    # Relaxed: 3-frame window, threshold 4.0 px/frame (allows more noise)
    velocity_stationarity_window_frames: int = 3
    velocity_stationarity_threshold_px_per_frame: float = 4.0


@dataclass
class CompletedAirborneEventV3_2:
    start_frame: int
    end_frame: int
    duration_frames: int
    fired_at_frame: int
...
class AirborneStateMachineV3_2:
    GROUND = "GROUND"
    LAUNCH_CANDIDATE = "LAUNCH_CANDIDATE"
    AIRBORNE = "AIRBORNE"

    def __init__(self, rule_config: AirborneRuleV2Config, sm_config: StateMachineV3_2Config):
        ...
        # V3_2: track recent dy values for velocity stationarity check
        self._recent_dy_values: list[float] = []
    ...
    def _update_ground_baseline(self, features_row):
        """V3_1 hybrid: primary deque, fallback to player baseline."""
        ...
        if (
            ...
            if len(self._ground_y_history) >= 5:
                deque_median = float(np.median(self._ground_y_history))
                self.ground_baseline_y = deque_median

        # Fallback to player baseline if deque not yet initialized
    ...
    def _is_velocity_stationary(self) -> bool:
        """V3_2: check if recent velocities are small (ball not rising/falling fast)."""
        if len(self._recent_dy_values) < self.sm_config.velocity_stationarity_window_frames:
            return False
        recent = self._recent_dy_values[-self.sm_config.velocity_stationarity_window_frames :]
        # Check if all recent dy values are below threshold (stationarity)
        for dy in recent:
            if isinstance(dy, float) and not np.isnan(dy):
                if abs(dy) > self.sm_config.velocity_stationarity_threshold_px_per_frame:
                    return False
        return True

    def _update_recent_dy(self, features_row):
        """Track recent dy values for velocity stationarity check."""
        dy = features_row.get("dy_per_frame_last_pair")
        if dy is not None and not (isinstance(dy, float) and np.isnan(dy)):
            self._recent_dy_values.append(float(dy))
            # Keep only last N values
            if len(self._recent_dy_values) > self.sm_config.velocity_stationarity_window_frames + 5:
                self._recent_dy_values.pop(0)
    ...
    def step(self, features_row) -> Optional[CompletedAirborneEventV3_2]:
        t = int(features_row["frame"])
        self._update_recent_rows(features_row)
        self._update_recent_dy(features_row)
        ...
        if self.state == self.GROUND:
            ...
            # V3_2: landing requires BOTH near-ground AND velocity stationarity
            if has_ascended and self._is_ball_near_ground(features_row) and self._is_velocity_stationary():
    ...
    def _complete_event(self, end_frame: int, reason: str) -> CompletedAirborneEventV3_2:
        start = int(self._active_event_start)
        end = int(end_frame)
        completed = CompletedAirborneEventV3_2(
            start_frame=start,
            end_frame=end,
...
def detect_airborne_events_with_state_machine_v3_2(
    features_df: pd.DataFrame,
    rule_config: AirborneRuleV2Config,
    sm_config: StateMachineV3_2Config,
) -> list[CompletedAirborneEventV3_2]:
    sm = AirborneStateMachineV3_2(rule_config, sm_config)
    completed_events = []
...
if __name__ == "__main__":
    ...
    events = detect_airborne_events_with_state_machine_v3_2(
        df,
        AirborneRuleV2Config(),
        StateMachineV3_2Config(),
    )
    for event in events:
        print(
