"""
Batch airborne event evaluation pipeline v3.

Uses v3 state machine with player-feet-derived per-frame ground baseline.
Replaces the stale-baseline deque median with per-frame player detection info.

Output: outputs/airborne_eval_v5_v3/ (separate from v2)
"""
...
DEFAULT_OUT_DIR = ROOT / "outputs" / "airborne_eval_v5_v3"

sys.path.insert(0, str(ROOT))
sys.path.insert(0, str(ED_DIR))

from airborne_rule_v2 import AirborneRuleV2Config
from airborne_state_machine_v3 import (
    StateMachineV3Config,
    detect_airborne_events_with_state_machine_v3,
)
...
def _plot_features(df, gt_events, clip_name, out_path):
    ...
    fig.suptitle(f"{clip_name} - trajectory features v3 (with player baseline)", y=1.0)
    ...
    panels = [
        ...
        ("player_ground_baseline_y", "px", "Player-feet ground baseline (v3)"),
...
def run_clip(clip_name: str, out_dir: Path, save_plots: bool) -> dict | None:
    ...
    print(f"\n{'=' * 60}")
    print(f"  {clip_name} v3 (with player baseline)")
    print(f"{'=' * 60}")
    ...
    completed = detect_airborne_events_with_state_machine_v3(df, rule_config, sm_config)
