"""
Trajectory feature extractor for airborne event detection v2.

This keeps the v1 no-pose/no-camera-motion contract, but adds low-latency
features that make launch detection less brittle when the ball briefly drops
out of the detector:

  - gap/reappearance features around the latest valid detection pair
  - short-window vertical motion summaries
  - causal 15-frame robust parabola fit features

All features use only detections at or before the current frame.
"""
...
def _fit_parabola_realtime(
    ...
    *,
    ...
    This mirrors the practical core of ransacp.py: fit a quadratic in a small
    frame window, score residual inliers, and optionally refit once on inliers.
    """
...
def extract_features(
    ...
    *,
    ...
    """
    For each frame t, compute causal trajectory features.

    The base columns are kept compatible with v1. New v2 columns are appended
    so downstream code can use richer signals without breaking old analysis.
    """
    if not detections:
        return pd.DataFrame()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default="trajectory_features_v2.csv")
    ...
    df = extract_features(dets, buffer_l=args.buffer_l, arc_window_frames=args.arc_window)
