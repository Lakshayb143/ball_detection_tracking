"""
Airborne state machine v2.

Adds a LAUNCH_CANDIDATE state between GROUND and AIRBORNE. This handles the
general detector-gap pattern:

  recent grounded detections -> short missing span -> ball reappears higher

The event is emitted when the reappearance confirms the launch, but the event
start is backdated to the first missing frame. No future lookahead, pose model,
or camera-motion estimate is used.
"""
...
@dataclass
class CompletedAirborneEventV2:
    start_frame: int
    end_frame: int
    duration_frames: int
    fired_at_frame: int
...
class AirborneStateMachineV2:
    GROUND = "GROUND"
    LAUNCH_CANDIDATE = "LAUNCH_CANDIDATE"
    AIRBORNE = "AIRBORNE"

    def __init__(self, rule_config: AirborneRuleV2Config, sm_config: StateMachineV2Config):
    ...
    def step(self, features_row) -> Optional[CompletedAirborneEventV2]:
        t = int(features_row["frame"])
    ...
    def _complete_event(self, end_frame: int, reason: str) -> CompletedAirborneEventV2:
        start = int(self._active_event_start)
        end = int(end_frame)
        completed = CompletedAirborneEventV2(
            start_frame=start,
            end_frame=end,
...
def detect_airborne_events_with_state_machine_v2(
    features_df: pd.DataFrame,
    rule_config: AirborneRuleV2Config,
    sm_config: StateMachineV2Config,
) -> list[CompletedAirborneEventV2]:
    sm = AirborneStateMachineV2(rule_config, sm_config)
    completed_events = []
...
if __name__ == "__main__":
    ...
    events = detect_airborne_events_with_state_machine_v2(
        df,
        AirborneRuleV2Config(),
