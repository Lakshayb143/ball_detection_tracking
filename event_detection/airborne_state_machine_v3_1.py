"""
Airborne state machine v3_1 (hybrid baseline).

Keeps v2's rolling deque as primary baseline source, but uses player-feet
detection (per-frame y2 percentile) as initialization and validation signal.

When deque is empty: initialize with player baseline (avoids cold start bias).
When deque exists: use it normally (tested, stable).
If player baseline diverges significantly from deque (>100px): indicates
possible camera pan; consider resetting deque to player baseline.

This hybrid approach solves testing_clip_1080's stale-baseline without
disrupting clip1/2's stable landing detection.
"""
...
class StateMachineV3_1Config:
    ...
    # V3_1 hybrid params
    player_baseline_divergence_threshold_px: float = 100.0


@dataclass
class CompletedAirborneEventV3_1:
    start_frame: int
    end_frame: int
    duration_frames: int
    fired_at_frame: int
...
class AirborneStateMachineV3_1:
    GROUND = "GROUND"
    LAUNCH_CANDIDATE = "LAUNCH_CANDIDATE"
    AIRBORNE = "AIRBORNE"

    def __init__(self, rule_config: AirborneRuleV2Config, sm_config: StateMachineV3_1Config):
        ...
        self._baseline_source: str = "none"  # Track source: "deque", "player", or "none"
    ...
    def _update_ground_baseline(self, features_row):
        """
        V3_1 hybrid: primary deque-based, with player baseline as fallback.

        Strategy:
        1. Always try to add ball ground detection to deque (v2 behavior)
        2. If deque is empty, use player baseline as initialization
        3. If player baseline available and diverges >100px from deque,
           note divergence (potential camera pan) but keep deque
        """
        # Primary: ball-based deque (v2 original)
        ...
        if (
            ...
            if len(self._ground_y_history) >= 5:
                ...
                self._baseline_source = "deque"

        # Secondary fallback: player baseline
        # Use only if deque is not yet initialized
    ...
    def step(self, features_row) -> Optional[CompletedAirborneEventV3_1]:
        t = int(features_row["frame"])
    ...
    def _complete_event(self, end_frame: int, reason: str) -> CompletedAirborneEventV3_1:
        start = int(self._active_event_start)
        end = int(end_frame)
        completed = CompletedAirborneEventV3_1(
            start_frame=start,
            end_frame=end,
...
def detect_airborne_events_with_state_machine_v3_1(
    features_df: pd.DataFrame,
    rule_config: AirborneRuleV2Config,
    sm_config: StateMachineV3_1Config,
) -> list[CompletedAirborneEventV3_1]:
    sm = AirborneStateMachineV3_1(rule_config, sm_config)
    completed_events = []
...
if __name__ == "__main__":
    ...
    events = detect_airborne_events_with_state_machine_v3_1(
        df,
        AirborneRuleV2Config(),
        StateMachineV3_1Config(),
    )
    for event in events:
        print(
