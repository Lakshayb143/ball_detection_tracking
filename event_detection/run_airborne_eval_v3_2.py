"""
Batch airborne event evaluation pipeline v3_2.

Uses v3_2 state machine: extends v3_1 hybrid baseline with velocity-stationarity landing confirmation.

Landing now requires BOTH:
1. Ball near baseline (distance < 50px)
2. Velocity stationarity: |dy| < 2.0 px/frame for last 5 frames

This prevents false landings when ball briefly passes near baseline mid-arc but is still moving.

Output: outputs/airborne_eval_v5_v3_2/ (separate from v2 and v3_1)
"""
...
PLAYER_DET_DIR = ROOT / "player_detections_v5"
GT_DIR = ROOT / "ground_truths"
DEFAULT_OUT_DIR = ROOT / "outputs" / "airborne_eval_v5_v3_2"

sys.path.insert(0, str(ROOT))
sys.path.insert(0, str(ED_DIR))

from airborne_rule_v2 import AirborneRuleV2Config
from airborne_state_machine_v3_2 import (
    StateMachineV3_2Config,
    detect_airborne_events_with_state_machine_v3_2,
)
...
from extract_trajectory_features_with_players_v3 import load_detections, load_player_detections, extract_features
...
def run_clip(clip_name: str, out_dir: Path) -> dict | None:
    det_path = DET_DIR / f"{clip_name}.json"
    player_det_path = PLAYER_DET_DIR / f"{clip_name}.json"
    ...
    if not det_path.exists():
        ...
        return None
    if not player_det_path.exists():
        print(f"[skip] {clip_name} - no player detection JSON at {player_det_path}")
        return None
    ...
    print(f"\n{'=' * 60}")
    print(f"  {clip_name} v3_2 (velocity-stationarity landing)")
    print(f"{'=' * 60}")

    detections = load_detections(str(det_path))
    player_detections = load_player_detections(str(player_det_path))
    ...
    print(
        f"  player detections loaded: "
        f"{sum(1 for v in player_detections.values() if v)} frames with players"
    )

    df = extract_features(detections, player_detections, buffer_l=30, arc_window_frames=15)
    ...
    completed = detect_airborne_events_with_state_machine_v3_2(df, rule_config, sm_config)
...
def write_summary(all_results: list[dict], out_dir: Path):
    ...
    print(f"\n{'=' * 60}")
    print("CONSOLIDATED SUMMARY V3_2")
    print(f"{'=' * 60}")
...
def main():
    ...
    for clip_name in find_clips(args.clips):
        result = run_clip(clip_name, out_dir=out_dir)
