#!/usr/bin/env python3
"""
RF-DETR primary + GroundingDINO fallback benchmark runner for SoccerNet SNMOT ball tracking.

This script isolates a second experiment:
1. RF-DETR is the primary detector.
2. The tracker keeps the outlier rejection logic from previous_works/ball-detection-without-states.py.
3. GroundingDINO is used only as the fallback path when RF-DETR fails or is rejected.
"""
...
DEFAULT_PLAYER_CLASS_IDS = "1,2,3"
DEFAULT_ENABLE_PLAYER_EXCLUSION = True
...
DEFAULT_GDINO_CROP_EXPANSION = 10.0
DEFAULT_GDINO_MIN_CROP_SIZE = 160
DEFAULT_GDINO_MAX_CROP_SIZE = 640
...
def is_point_in_boxes(point: np.ndarray, boxes: np.ndarray) -> bool:
    if boxes.size == 0:
        return False
    return bool(
        np.any(
...
def load_sequence_fps(seq_dir: Path, default_fps: float) -> float:
    ...
    if parser.has_section("Sequence") and parser.has_option("Sequence", "frameRate"):
        try:
...
class RFDETRGroundingDinoTracker:
    def __init__(self, fps: float, args: argparse.Namespace) -> None:
        self.fps = fps
        self.args = args
        self.kf = AdaptiveKalmanFilter(dt=1.0 / max(fps, 1.0))
        ...
        self.last_box_wh: Optional[np.ndarray] = None
        self.predicted_position: Optional[np.ndarray] = None

    def predict(self) -> Optional[np.ndarray]:
        self.predicted_position = None
        if not self.track_initialized or not self.kf.initialized:
            return None
        self.predicted_position = self.kf.predict()
        return self.predicted_position.copy()
    ...
    def select_best_candidate(
        ...
        if not candidates:
            return None
        if predicted_position is None or not self.track_initialized:
            return max(candidates, key=lambda item: item.score)

        gated: List[Tuple[float, DetectionCandidate]] = []
        for candidate in candidates:
            distance = float(np.linalg.norm(xyxy_center(candidate.xyxy) - predicted_position))
            if distance < self.args.validation_gate_threshold:
                gated.append((distance, candidate))
    ...
    def try_accept_candidate(self, candidate: DetectionCandidate) -> bool:
        ...
        if should_use:
            ...
            self.track_lost_count = 0
            self.kf.set_process_noise(1.0 if self.track_hit_streak > self.args.stable_track_threshold else 10.0)
            ...
            self.last_box_wh = xyxy_wh(candidate.xyxy)
            return True
    ...
    def reset(self) -> None:
        self.kf = AdaptiveKalmanFilter(dt=1.0 / max(self.fps, 1.0))
        self.outlier_detector.reset()
        self.track_initialized = False
        self.track_lost_count = 0
        self.track_hit_streak = 0
        self.prev_position = None
        self.last_box_wh = None
        self.predicted_position = None

    def build_fallback_crop(self, frame_shape: Tuple[int, int, int]) -> Optional[np.ndarray]:
...
def exclude_candidates_in_boxes(candidates: Sequence[DetectionCandidate], boxes: np.ndarray) -> List[DetectionCandidate]:
    if len(candidates) == 0 or boxes.size == 0:
        return list(candidates)
    return [candidate for candidate in candidates if not is_point_in_boxes(xyxy_center(candidate.xyxy), boxes)]
...
def build_trackeval_files(
    ...
    tracker_name = "rfdetr_gdino_fallback_tracker"
    ...
    for frame_id, image_path in enumerate(image_paths, start=1):
        ...
        if accepted is None and args.enable_gdino_fallback and gdino_detector is not None:
            ...
            if crop_xyxy is not None:
                fallback_candidates = gdino_detector.predict_candidates_in_crop(image, crop_xyxy)
            elif args.gdino_full_frame_on_reset:
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="RF-DETR primary + GroundingDINO fallback benchmark runner for SoccerNet SNMOT ball tracking."
    )
    ...
    parser.add_argument("--gdino_crop_expansion", type=float, default=DEFAULT_GDINO_CROP_EXPANSION)
    parser.add_argument("--gdino_min_crop_size", type=int, default=DEFAULT_GDINO_MIN_CROP_SIZE)
