...
import ball_outlier_interpolator_v4 as v4  # noqa: E402


def _resolved_default_path(path_value: str | Path) -> Path:
...
DEFAULT_RUN_NAME = "ball_outlier_interpolator_v4__clip1"
DEFAULT_SEQ_START = 0
DEFAULT_SEQ_END = 0
DEFAULT_SEQ_LIST = ""
DEFAULT_MAX_FRAMES_PER_SEQ = 0

DEFAULT_BALL_MODEL_PATH = _resolved_default_path(v4.MODEL_PATH)
DEFAULT_PLAYER_MODEL_PATH = _resolved_default_path(v4.PLAYER_MODEL_PATH)
DEFAULT_BALL_MODEL_RESOLUTION = int(v4.MODEL_RESOLUTION)
DEFAULT_BALL_CONFIDENCE = float(v4.CONFIDENCE)
DEFAULT_PLAYER_CONFIDENCE = float(v4.PLAYER_CONFIDENCE)
DEFAULT_BALL_CLASS_ID = int(v4.BALL_CLASS_ID)
DEFAULT_PLAYER_REJECTION_CLASS_IDS = ",".join(str(item) for item in v4.PLAYER_REJECTION_CLASS_IDS)
DEFAULT_ENABLE_RFDETR_OPTIMIZE = bool(v4.ENABLE_RFDETR_OPTIMIZE)
DEFAULT_ACTIONS_JSON_PATH = _resolved_default_path(v4.ACTIONS_JSON_PATH)

DEFAULT_FPS_ASSUMPTION = float(v4.DEFAULT_FPS_IF_MISSING)
DEFAULT_INTERPOLATED_BOX_SIZE = float(v4.BENCHMARK_INTERPOLATED_BOX_SIZE)
DEFAULT_MATCH_IOU = float(v4.BENCHMARK_MATCH_IOU)
DEFAULT_EVAL_SCORE_THRESHOLD = float(v4.BENCHMARK_EVAL_SCORE_THRESHOLD)
DEFAULT_BALL_CATEGORY_ID = int(v4.BENCHMARK_BALL_CATEGORY_ID)
...
class FrameRecord:
    ...
    player_rejection_bbox_count: int
    ...
    regime_name: str
    max_gap: int
    phase1_active_frame: bool
    phase1_rejections: int
    tracker_active: bool
    gap_frames: int
    hit_streak: int
...
class V4TrackerAdapter:
    """Keeps v4 tracking behavior intact while letting us benchmark image folders."""
    ...
    def __init__(
        ...
        actions_json_path: Optional[Path],
    ) -> None:
        self.confidence_threshold = float(confidence_threshold)
        self.mahalanobis_gate = float(mahalanobis_gate)
        self.kf = v4.BallKalmanFilter(dt=1.0 / max(1e-6, float(fps_assumption)))
        self.outlier_confirmer = v4.OutlierConfirmer(
            max_consecutive=int(outlier_confirm_frames)
        )

        action_path_value = str(actions_json_path) if actions_json_path is not None else None
        self.action_timeline = v4.ActionTimeline(action_path_value)
        self.current_max_gap = v4.REGIME_DYNAMICS["Default"].max_gap
    ...
    def apply_regime(self, frame_count: int) -> v4.ActionRegime:
        regime = self.action_timeline.get_regime(frame_count)

        if v4.ENABLE_PHASE_2:
            dynamics = v4.REGIME_DYNAMICS.get(regime.name, v4.REGIME_DYNAMICS["Default"])
        else:
            dynamics = v4.RegimeDynamics(
                max_gap=v4.BASELINE_MAX_GAP_FRAMES,
                q_velocity=v4.BASELINE_Q_VELOCITY,
                q_acceleration=v4.BASELINE_Q_ACCELERATION,
            )
    ...
    def select_best_detection(
        ...
        # Phase 1 — gated on flag
        if v4.ENABLE_PHASE_1 and regime.name in v4.AIRBORNE_REGIMES and len(player_bboxes) > 0:
            self.phase1_active_frames += 1
            keep_indices = []
            for i, center in enumerate(centers):
                if not v4.detection_inside_any_player(center, player_bboxes):
        ...
        if np.any(in_gate_mask):
            ...
            best = int(in_gate_indices[np.argmin(m_dists[in_gate_indices])])
    ...
    def step(
        ...
        if should_reset:
            ...
            self.hit_streak = 1
            self.outlier_confirmer.reset()
            print(f"Frame {frame_count}: RESET at {center.round(1)}")
            return center, False, best_detection["sv"]
...
def aggregate_sequence_summaries(summaries: Sequence[SequenceSummary]) -> Dict[str, float]:
    ...
    totals = {
        ...
        "phase1_active_frames": 0.0,
        "phase1_rejections": 0.0,
    ...
    totals["player_ms_avg"] = mean_or_zero([item.player_ms_avg for item in summaries])
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark ball_outlier_interpolator_v4.py on clip1-style frame folders while "
    ...
    parser.add_argument(
        "--player_rejection_class_ids",
        type=str,
        default=DEFAULT_PLAYER_REJECTION_CLASS_IDS,
        help="Comma-separated player detector class IDs used for inside-player rejection.",
    )
    ...
    parser.add_argument("--mahalanobis_gate", type=float, default=float(v4.MAHALANOBIS_GATE))
    parser.add_argument("--outlier_confirm_frames", type=int, default=int(v4.OUTLIER_CONFIRM_FRAMES))
    parser.add_argument("--actions_json", type=Path, default=DEFAULT_ACTIONS_JSON_PATH)
...
def _player_rejection_bboxes(
    player_detections: sv.Detections,
    player_rejection_class_ids: tuple[int, ...],
) -> np.ndarray:
    if len(player_detections) == 0:
        return np.empty((0, 4), dtype=np.float32)
    keep = np.isin(player_detections.class_id, player_rejection_class_ids)
    if not np.any(keep):
        return np.empty((0, 4), dtype=np.float32)
    return player_detections.xyxy[keep].astype(np.float32)
...
def process_sequence(
    ...
    for frame_id, image_path in enumerate(sequence.image_paths, start=1):
        ...
        regime = tracker.apply_regime(frame_id)

        primary_start = time.perf_counter()
        detections = ball_model.predict(image, confidence=args.ball_confidence)
        ...
        player_detections = player_model.predict(image, confidence=args.player_confidence)
        player_ms = (time.perf_counter() - player_start) * 1000.0
        player_bboxes = _player_rejection_bboxes(
            player_detections=player_detections,
            player_rejection_class_ids=args.player_rejection_class_ids,
        )
        ...
        phase1_active_before = tracker.phase1_active_frames
        phase1_rejections_before = tracker.phase1_rejections
        tracking_start = time.perf_counter()
        best_detection = tracker.select_best_detection(
        ...
        frame_records.append(
            FrameRecord(
                ...
                player_candidate_count=int(len(player_detections.xyxy)),
                player_rejection_bbox_count=int(len(player_bboxes)),
                selected_score=selected_score,
                selected_in_gate=selected_in_gate,
                output_score=output_score,
                output_source=output_source,
                regime_name=regime.name,
                max_gap=int(tracker.current_max_gap),
                phase1_active_frame=bool(tracker.phase1_active_frames > phase1_active_before),
                phase1_rejections=int(tracker.phase1_rejections - phase1_rejections_before),
    ...
    summary = SequenceSummary(
        ...
        phase1_active_frames=sum(1 for item in frame_records if item.phase1_active_frame),
        phase1_rejections=sum(item.phase1_rejections for item in frame_records),
        ...
        player_ms_avg=mean_or_zero([item.player_ms for item in frame_records]),
...
def main() -> None:
    ...
    args.actions_json = args.actions_json.expanduser().resolve() if args.actions_json is not None else None
    args.player_rejection_class_ids = _parse_int_tuple(args.player_rejection_class_ids)
    ...
    print(f"[INFO] Ball model resolution: {args.ball_model_resolution}")
    print(f"[INFO] Ball confidence threshold: {args.ball_confidence}")
    print(f"[INFO] Player model path: {args.player_model_path}")
    print(f"[INFO] Player confidence threshold: {args.player_confidence}")
    print(f"[INFO] Player rejection class IDs: {args.player_rejection_class_ids}")
    print(
        f"[INFO] v4 tracker config: gate={args.mahalanobis_gate}, "
        f"outlier_confirm_frames={args.outlier_confirm_frames}"
    )
    print(f"[INFO] Actions JSON: {args.actions_json}")
    print(f"[INFO] FPS assumption: {args.fps_assumption}")
    print(f"[INFO] Interpolated box size: {args.interpolated_box_size}")
    print(f"[INFO] v4.ENABLE_PHASE_1 = {v4.ENABLE_PHASE_1}")
    print(f"[INFO] v4.ENABLE_PHASE_2 = {v4.ENABLE_PHASE_2}")
    ...
    if args.enable_rfdetr_optimize:
        try:
            ball_model.optimize_for_inference()
            print("[INFO] Optimized ball RF-DETR model for inference")
        except Exception as exc:
            print(f"[WARN] Could not optimize ball RF-DETR model: {exc}")

    player_model = RFDETRMedium(pretrain_weights=str(args.player_model_path))
    if args.enable_rfdetr_optimize:
        try:
            player_model.optimize_for_inference()
            print("[INFO] Optimized player RF-DETR model for inference")
        except Exception as exc:
            print(f"[WARN] Could not optimize player RF-DETR model: {exc}")
    ...
    for sequence in sequences:
        ...
        print(
            ...
            f"phase1_rejections={summary.phase1_rejections}, fps={summary.runtime_fps:.2f}"
        )
    ...
    experiment_summary = {
        ...
        "aggregate": aggregate,
        "logic_reference": str((REPO_ROOT / "ball_outlier_interpolator_v4.py").resolve()),
        "logic_notes": {
            "core_tracking_logic": "Copied from ball_outlier_interpolator_v4.py without intended behavior changes.",
            ...
            "actions_json": str(args.actions_json) if args.actions_json is not None else None,
            "phase1": "Airborne-regime detections whose center falls inside player/referee boxes are rejected.",
            "phase2": "Action regimes set Kalman process noise and max interpolation gap.",
        },
    }
