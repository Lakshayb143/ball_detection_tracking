...
import ball_outlier_interpolator_v3 as v3  # noqa: E402
...
DEFAULT_RUN_NAME = "ball_outlier_interpolator_v3__clip1"
DEFAULT_SEQ_START = 0
DEFAULT_SEQ_END = 0
DEFAULT_SEQ_LIST = ""
DEFAULT_MAX_FRAMES_PER_SEQ = 0

DEFAULT_BALL_MODEL_PATH = _resolved_default_path(v3.MODEL_PATH)
DEFAULT_BALL_MODEL_RESOLUTION = int(v3.MODEL_RESOLUTION)
DEFAULT_BALL_CONFIDENCE = float(v3.CONFIDENCE)
DEFAULT_BALL_CLASS_ID = int(v3.BALL_CLASS_ID)
DEFAULT_ENABLE_RFDETR_OPTIMIZE = bool(v3.ENABLE_RFDETR_OPTIMIZE)
DEFAULT_ACTIONS_JSON_PATH = _resolved_default_path(v3.ACTIONS_JSON_PATH)
DEFAULT_ACTION_REJECTION_THRESHOLD = float(v3.ACTION_REJECTION_THRESHOLD)

DEFAULT_FPS_ASSUMPTION = float(v3.DEFAULT_FPS_IF_MISSING)
DEFAULT_INTERPOLATED_BOX_SIZE = float(v3.BENCHMARK_INTERPOLATED_BOX_SIZE)
DEFAULT_MATCH_IOU = float(v3.BENCHMARK_MATCH_IOU)
DEFAULT_EVAL_SCORE_THRESHOLD = float(v3.BENCHMARK_EVAL_SCORE_THRESHOLD)
DEFAULT_BALL_CATEGORY_ID = int(v3.BENCHMARK_BALL_CATEGORY_ID)
...
class V3TrackerAdapter:
    """Keeps v3 tracking behavior intact while letting us benchmark image folders."""
    ...
    def __init__(
        ...
        action_rejection_threshold: float,
    ) -> None:
        self.confidence_threshold = float(confidence_threshold)
        self.mahalanobis_gate = float(mahalanobis_gate)
        self.max_gap_frames = int(max_gap_frames)
        self.action_rejection_threshold = float(action_rejection_threshold)

        self.kf = v3.BallKalmanFilter(dt=1.0 / max(1e-6, float(fps_assumption)))
        self.outlier_confirmer = v3.OutlierConfirmer(max_consecutive=int(outlier_confirm_frames))

        action_path_value: Optional[str]
        if actions_json_path is None:
            action_path_value = None
        else:
            action_path_value = str(actions_json_path)
        self.action_timeline = v3.ActionTimeline(action_path_value)
        self.action_filter = v3.ActionConditionalFilter(frame_height=int(frame_height))

        self.track_initialized = False
        self.frames_since_detection = 0
        self.hit_streak = 0

    def get_regime_name(self, frame_count: int) -> str:
        return self.action_timeline.get_regime(frame_count).name

    def select_best_detection(self, ball_detections: sv.Detections, frame_count: int) -> Optional[dict]:
        ...
        action_scores = np.array(
            [
                self.action_filter.score(center, predicted_xy, predicted_vel, regime)
        ...
        valid_action_mask = action_scores < self.action_rejection_threshold
        ...
        in_gate_mask = m_dists[valid_indices] < self.mahalanobis_gate
        if np.any(in_gate_mask):
            in_gate_indices = valid_indices[in_gate_mask]
        ...
        best = int(valid_indices[np.argmax(filtered.confidence[valid_indices])])
    ...
    def step(
        ...
        if best_detection is None:
            ...
            if self.frames_since_detection > self.max_gap_frames:
                ...
                print(f"Frame {frame_count}: track lost - gap exceeded {self.max_gap_frames} frames")
                return None, False, None
            print(
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark ball_outlier_interpolator_v3.py on clip1-style frame folders while "
    ...
    parser.add_argument("--mahalanobis_gate", type=float, default=float(v3.MAHALANOBIS_GATE))
    parser.add_argument("--max_gap_frames", type=int, default=int(v3.MAX_GAP_FRAMES))
    parser.add_argument("--outlier_confirm_frames", type=int, default=int(v3.OUTLIER_CONFIRM_FRAMES))
    parser.add_argument("--actions_json", type=Path, default=DEFAULT_ACTIONS_JSON_PATH)
    parser.add_argument("--action_rejection_threshold", type=float, default=DEFAULT_ACTION_REJECTION_THRESHOLD)
...
def process_sequence(
    ...
    first_image = cv2.imread(str(sequence.image_paths[0]))
    if first_image is None:
        raise RuntimeError(f"Could not read first frame {sequence.image_paths[0]}")
    tracker = V3TrackerAdapter(
        ...
        action_rejection_threshold=args.action_rejection_threshold,
    )
    ...
    for frame_id, image_path in enumerate(sequence.image_paths, start=1):
        ...
        regime_name = tracker.get_regime_name(frame_id)
        best_detection = tracker.select_best_detection(ball_detections, frame_id)
...
def main() -> None:
    ...
    print(
        f"[INFO] v3 tracker config: gate={args.mahalanobis_gate}, "
        f"max_gap_frames={args.max_gap_frames}, outlier_confirm_frames={args.outlier_confirm_frames}, "
        f"action_rejection_threshold={args.action_rejection_threshold}"
    )
    ...
    experiment_summary = {
        ...
        "aggregate": aggregate,
        "logic_reference": str((REPO_ROOT / "ball_outlier_interpolator_v3.py").resolve()),
        "logic_notes": {
            "core_tracking_logic": "Copied from ball_outlier_interpolator_v3.py without intended behavior changes.",
            ...
            "action_rejection_threshold": float(args.action_rejection_threshold),
        },
    }
