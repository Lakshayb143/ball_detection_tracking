#!/usr/bin/env python3
"""
RF-DETR ball-only + outlier rejection + GroundingDINO full-frame fallback benchmark runner.
...
5. GroundingDINO runs on the full frame as the fallback whenever RF-DETR gives no accepted ball.
"""
...
class RFDetrBallDetector:
    def __init__(
        ...
        model_kwargs = {"pretrain_weights": str(ball_model_path)}
        if ball_model_resolution is not None and int(ball_model_resolution) > 0:
            model_kwargs["resolution"] = int(ball_model_resolution)
        self.ball_model = RFDETRMedium(**model_kwargs)
    ...
    def predict_ball_candidates(self, image_bgr: np.ndarray) -> List[DetectionCandidate]:
        ...
        for index in range(num_items):
            ...
            if score < float(self.ball_confidence):
                continue
            candidates.append(
                DetectionCandidate(
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="RF-DETR ball-only + outlier rejection + GroundingDINO full-frame fallback benchmark runner."
    )
...
def main() -> None:
    ...
    print(f"[INFO] GroundingDINO full-frame fallback enabled: {args.enable_gdino_fallback}")
