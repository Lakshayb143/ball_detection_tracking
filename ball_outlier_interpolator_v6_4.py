...
OUTPUT_PATH = "/home/lakshay/lx/ball_detection_tracking/clip1_output_v6_4.mp4"
DETECTION_JSON_PATH = "/home/lakshay/lx/ball_detection_tracking/clip1_v6_4.json"
...
# V6_4: white-patch rejection for interpolated positions.
# Instead of rejecting green grass, we check if the patch is white/light (ball-like).
# If not white enough, it's likely on a colored surface (jersey, sunlit grass, etc) → reject.
ENABLE_WHITE_FILTER = True
WHITE_FILTER_THRESHOLD = 0.1  # require >= 50% white pixels to accept interpolation
WHITE_CROP_MARGIN = 0.1       # crop radius = margin × (max(bw, bh) / 2)
WHITE_CROP_FALLBACK_RADIUS = 1.0  # used until first accepted detection seen
# HSV ranges for white/light pixels (OpenCV H in 0-180).
# White: any H, low S (< 40), high V (> 150)
WHITE_HSV_LOWER = (0, 0, 150)
...
# ============================================================
# V6_4: white-patch filter for interpolated positions
# ============================================================
def is_white_patch(frame: np.ndarray, position, crop_radius: float) -> bool:
    """Return True if >= WHITE_FILTER_THRESHOLD of crop is white/light (ball-like color)."""
    ...
    lower = np.array(WHITE_HSV_LOWER, dtype=np.uint8)
    upper = np.array(WHITE_HSV_UPPER, dtype=np.uint8)
    mask = cv2.inRange(hsv, lower, upper)
    white_fraction = float(np.count_nonzero(mask)) / float(mask.size)
    return white_fraction >= WHITE_FILTER_THRESHOLD
...
class VideoProcessor:
    def __init__(self):
        ...
        self.white_rejections = 0

        # V6_4: tracks crop radius from last accepted detection bbox
        self.last_ball_crop_radius = WHITE_CROP_FALLBACK_RADIUS
    ...
    def run(self):
        ...
        while True:
            ...
            # V6_4: update crop radius from accepted detection bbox.
            if accepted is not None and len(accepted.xyxy) > 0:
                x1, y1, x2, y2 = accepted.xyxy[0]
                bw, bh = float(x2 - x1), float(y2 - y1)
                self.last_ball_crop_radius = max(bw, bh) / 2.0 * WHITE_CROP_MARGIN

            # V6_4: suppress interpolated positions that are NOT white/light (likely FP).
            if ENABLE_WHITE_FILTER and is_interpolated and output_position is not None:
                if not is_white_patch(frame, output_position, self.last_ball_crop_radius):
                    print(
                        f"Frame {frame_count}: WHITE REJECT interpolated at "
                        f"{output_position.round(1)} "
                        f"(crop_r={self.last_ball_crop_radius:.1f}px)"
                    )
                    self.white_rejections += 1
        ...
        print(
            f"V6_4 white filter: rejected {self.white_rejections} interpolated position(s) "
            f"(threshold={WHITE_FILTER_THRESHOLD:.0%})"
        )
