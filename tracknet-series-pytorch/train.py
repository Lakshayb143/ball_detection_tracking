...
from model.tracknet_v2 import TrackNet as TrackNetV2
from model.tracknet_v4 import TrackNet as TrackNetV4
...
class Trainer:
    ...
    def _setup_wandb(self):
        ...
        if not self.is_main:
            return
        wandb.init(
            project=wandb_cfg.get("project", "tracknet"),
            entity=wandb_cfg.get("entity"),
            name=f"{self.cfg['name']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            config={
                "model": self.cfg.get("model", "v2"),
    ...
    def setup_model(self):
        dropout = self.cfg.get('dropout', 0.0)
        model_version = self.cfg.get('model', 'v2')
        ModelClass = TrackNetV4 if model_version == 'v4' else TrackNetV2
        self.model = ModelClass(dropout=dropout).to(self.device)
        if self.is_main:
            total_params = sum(p.numel() for p in self.model.parameters())
            print(f"Model: TrackNet {model_version.upper()} ({total_params:,} params)")
        if self.world_size > 1:
